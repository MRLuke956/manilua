...
class SteamVerification:    
    def __init__(self):
        ...
        # Lightweight cache to avoid heavy per-request computations
        self._cache_ttl_secs = 10
        self._last_process_hash_ts = 0.0
        self._last_process_hash_val: Optional[str] = None
        self._last_memory_proof_ts = 0.0
        self._last_memory_proof_val: Optional[str] = None
    ...
    def _get_process_hash(self) -> str:
        # TTL cache
        now = time.time()
        if self._last_process_hash_val and (now - self._last_process_hash_ts) < self._cache_ttl_secs:
            return self._last_process_hash_val
        try:
            if self.steam_process:
                ...
                val = hashlib.sha256(process_data.encode()).hexdigest()[:32]
                self._last_process_hash_val = val
                self._last_process_hash_ts = now
                return val
        except Exception:
            pass
        fallback_data = f"{now}-{self.steam_pid}"
        val = hashlib.sha256(fallback_data.encode()).hexdigest()[:32]
        self._last_process_hash_val = val
        self._last_process_hash_ts = now
        return val
    ...
    def _get_memory_proof(self) -> str:
        # TTL cache
        now = time.time()
        if self._last_memory_proof_val and (now - self._last_memory_proof_ts) < self._cache_ttl_secs:
            return self._last_memory_proof_val
        try:
            if self.steam_process:
                threads = len(self.steam_process.threads())
                # Avoid memory_maps() due to high overhead
                memory_data = f"{threads}-{self.steam_pid}"
                val = hashlib.sha256(memory_data.encode()).hexdigest()[:32]
                self._last_memory_proof_val = val
                self._last_memory_proof_ts = now
                return val
        except Exception:
            pass
        fallback_data = f"memory-{self.steam_pid}-{now}"
        val = hashlib.sha256(fallback_data.encode()).hexdigest()[:32]
        self._last_memory_proof_val = val
        self._last_memory_proof_ts = now
        return val
